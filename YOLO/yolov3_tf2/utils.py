...
from yolov3_tf2.inference import production
...
def draw_outputs(img, outputs, class_names):
    ...
    font = ImageFont.truetype(font='./data/fonts/futur.ttf',
                              size=(img.size[0] + img.size[1]) // 30)
    for i in range(nums):
        color = colors[int(classes[i])]
        x1y1 = ((np.array(boxes[i][0:2]) * wh).astype(np.int32))
        x2y2 = ((np.array(boxes[i][2:4]) * wh).astype(np.int32))
        thickness = (img.size[0] + img.size[1]) // 100
        x0, y0 = x1y1[0], x1y1[1]
        if (objectness[i]*100) > 50:
            cropped_img = img.crop([x1y1[0]+10, x1y1[1]-20, x2y2[0]+20, x2y2[1]+20])
            ...
            temp = production()
            temp = temp.validation(img = cropped_img)
